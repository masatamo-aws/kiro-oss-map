...
/**
 * Route service for calculating routes using OSRM
 */
export class RouteService {
  ...
  async calculateRoute(origin, destination, profile = 'driving', routeOptions = {}) {
    ...
    try {
      ...
      if (profile === 'walking') {
        route = await this.calculateWalkingRoute(origin, destination, routeOptions);
      } else {
        route = await this.calculateDrivingRoute(origin, destination, routeOptions);
      }
      ...
      Logger.error('Route calculation failed', error, 'route-calculation');
      throw error;
    }
  }
